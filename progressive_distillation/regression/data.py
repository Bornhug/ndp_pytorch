...
def _randn_like(x: torch.Tensor, g: torch.Generator) -> torch.Tensor:
    # Older torch may not support randn_like with generator; always use shape-based variant
...
def _rand_like(x: torch.Tensor, g: torch.Generator) -> torch.Tensor:
    # Likewise for rand_like; use shape-based variant for broad compatibility
...
def get_batch(
    ...
    *,
    ...
    y_context, y_target = y_all[:, :n_context], y_all[:, n_context:]  # y_context, y_target here are both prior samples
    ...
    if gp_conditional_targets and _DATASET_CONFIGS[name].is_gp and hasattr(function_distribution, "kernel_fn"):
        ...
        for b in range(batch_size):
            ...
            if M == 0: # No need, because targets are already prior
