...
class DDIMSampler:
    ...
    def __post_init__(self):
        T = int(self.process.alpha_bars.numel())
        ts = torch.linspace(T - 1, 0, steps=self.num_sample_steps)
        idx = ts.round().to(torch.long)
        # keep DESCENDING order (T-1 → … → 0), remove only consecutive dupes
        self.timesteps = torch.unique_consecutive(idx)
        if self.timesteps[-1].item() != 0:
            self.timesteps = torch.cat([self.timesteps, torch.tensor([0], device=idx.device)])
    ...
    @torch.no_grad()
    def sample_cond(self, key, x_query, mask_tgt, *, x_context, y_context, mask_context, model_fn, y_dim=1):
        device = x_query.device
        if mask_tgt is None:     mask_tgt = torch.zeros(x_query.size(0), device=device)
        if mask_context is None: mask_context = torch.ones(x_context.size(0), device=device)  # known!

        abars = self.process.alpha_bars.to(device)
        num_ctx = x_context.size(0)
        y_t = torch.randn(x_query.size(0), y_dim, device=device, generator=key)
        z_ctx = torch.randn(y_context.shape, dtype=y_context.dtype ,device=y_context.device, generator=key)  # fixed per sample
        ...
        for i in range(len(self.timesteps) - 1):
            ...
            y_ctx_t = a_i * y_context + ai1 * z_ctx  # forward-noised context at t_i

            x_aug = torch.cat([x_context, x_query], dim=0)
            mask_aug = torch.cat([mask_context, mask_tgt], dim=0)
            y_aug = torch.cat([y_ctx_t, y_t], dim=0)

            y_aug_next = self._ddim_step(y_aug, x_aug, mask_aug, t_i, t_j, model_fn, key)

            # clamp context to the correct forward value at t_j
            a_j = torch.sqrt(abars[t_j]);
            aj1 = torch.sqrt(1.0 - abars[t_j])
            y_ctx_t_next = a_j * y_context + aj1 * z_ctx
            y_t = torch.cat([y_ctx_t_next, y_aug_next[num_ctx:]], dim=0)[num_ctx:]
    ...
    #         eps_ctx = torch.randn(y_context.shape, dtype=y_context.dtype, device=y_context.device, generator=key)
    #
