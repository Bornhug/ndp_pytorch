...
def _randn_like(
    ...
    *,
    ...
    """
    if generator is not None and hasattr(generator, "randn_like"):
        return generator.randn_like(ref)
    try:
...
class GaussianDiffusion:
    ...
    def _normalize_sampling_method(method: str) -> str:
        ...
        if method not in {"ddpm", "ddim", "repaint"}:
    ...
    def repaint_forward_jump(
        ...
        *,
        ...
        """Forward-noise a RePaint target state from ``from_t`` back to ``to_t``."""
        if from_t < 0 or to_t <= from_t:
            return yt
        alpha_from = _expand_to(
            self.alpha_bars[int(from_t)].to(device=yt.device, dtype=yt.dtype),
            yt,
        )
        alpha_to = _expand_to(
            self.alpha_bars[int(to_t)].to(device=yt.device, dtype=yt.dtype),
            yt,
        )
        alpha_ratio = (alpha_to / alpha_from.clamp(min=1e-8)).clamp(
            min=0.0,
            max=1.0,
        )
        return (
            torch.sqrt(alpha_ratio) * yt
            + torch.sqrt((1.0 - alpha_ratio).clamp(min=0.0))
            * _randn_like(yt, generator=key)
        )
    ...
    def conditional_sample(
        ...
        *,
        ...
        """Condition an unconditional model on observed rows via RePaint.

        Context labels are forward-noised to the current reverse timestep and
        concatenated with the current target sample. The model denoises the
        whole augmented task, while only the target rows are kept for the next
        step.
        """
        method = self._normalize_sampling_method(method)
        reverse_method = "ddpm" if method == "repaint" else method
        batch_size, num_target, _ = x.shape
        num_context = x_context.shape[1]
        output_dim = y_context.shape[-1]
        device = x.device
        dtype = x.dtype

        x_aug = torch.cat([x_context, x], dim=1)
        augmented_mask = torch.zeros(
            (batch_size, num_context + num_target),
            dtype=torch.bool,
            device=device,
        )
        y_t = _randn_like(
        ...
        schedule = _build_reverse_schedule(
            len(self.betas),
            num_sample_steps if reverse_method == "ddim" else None,
        )
        amp_enabled = (
            bool(amp)
        ...
        def reverse_augmented(target_state: torch.Tensor, t_int: int, next_t: int):
            t_batch = torch.full(
                (batch_size,),
                t_int,
                device=device,
            ...
            t_context = t_batch.view(batch_size, 1, 1)
            # A new context-noise draw is used on every RePaint denoising call.
            y_context_t, _ = self.forward(key, y_context, t_context)
            y_aug = torch.cat([y_context_t.to(dtype=dtype), target_state], dim=1)
            with torch.autocast(
                device_type="cuda",
                dtype=amp_dtype or torch.float16,
                enabled=amp_enabled,
                ...
                eps_hat = model(
                    ...
                    y_target=y_aug,
                    t=t_batch,
                    mask_target=augmented_mask,
                ).float()
            previous = self.reverse_step(
                key,
                eps_hat,
                y_aug,
                t_int,
            ...
            return previous[:, num_context:, :]

        for step_idx, t_int in enumerate(schedule):
            next_t = schedule[step_idx + 1] if step_idx + 1 < len(schedule) else -1
            # Match original RePaint: each inner iteration goes backward and
            # then forward again, followed by one final backward transition.
            if next_t >= 0:
                for _ in range(max(int(num_inner_steps), 0)):
                    previous_target = reverse_augmented(y_t, t_int, next_t)
                    y_t = self.repaint_forward_jump(
                        key,
                        previous_target,
                        from_t=next_t,
                        to_t=t_int,
                    )
            y_t = reverse_augmented(y_t, t_int, next_t)
